...
class CaptchaView(APIView):
    ...
    @extend_schema(
        responses={200: OpenApiResponse(description="Retrieved successfully")},
        auth=[],
        operation_id="captcha-get",
        description="Captcha retrieval",
    )
...
class LoginSerializer(TokenObtainPairSerializer):
    """
    Login serializer:
    Overrides djangorestframework-simplejwt serializer
    """
    ...
    def validate(self, attrs):
        ...
        if dispatch.get_system_config_values("base.captcha_state"):
            ...
            if self.image_code and five_minute_ago > self.image_code.expiration:
                ...
                raise CustomValidationError("Captcha has expired")
            else:
                if self.image_code and (
                    ...
                    self.image_code and self.image_code.delete()
                    raise CustomValidationError("Image captcha error")

        try:
            user = Users.objects.get(username=attrs['username'])
        except Users.DoesNotExist:
            raise CustomValidationError("Account/password error")
        if not user.is_active:
            raise CustomValidationError("Account is locked, please contact admin to unlock")
        try:
            ...
            raise CustomValidationError(f"Account/password error; will be locked after {count} more retries")
...
class LoginView(TokenObtainPairView):
    ...
    #     # username may carry more than just username, may include other unique identifiers like phone/email
...
class LoginTokenSerializer(TokenObtainPairSerializer):
    ...
    default_error_messages = {"no_active_account": _("Account/password incorrect")}
...
class ApiLoginSerializer(CustomModelSerializer):
    """API documentation login - serializer"""
...
class ApiLogin(APIView):
    """API documentation login interface"""
