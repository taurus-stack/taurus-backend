...
IGNORE_DATA_TABLES=(
    "test_taurus_backend.taurus_system_operation_log"
)
...
# 导出文件名
DUMP_FILE="test_taurus_backend.dump.sql"
DATE_DUMP_FILE="test_taurus_backend.dump.${DATE}.sql"

# 临时文件
SCHEMA_FILE=$(mktemp /tmp/test_taurus_schema.XXXXXX.sql)
DATA_FILE=$(mktemp /tmp/test_taurus_data.XXXXXX.sql)
...
echo "=== 开始导出 test_taurus_backend 测试数据库 ==="
